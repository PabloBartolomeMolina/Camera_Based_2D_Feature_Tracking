...
int main(int argc, const char *argv[])
{
    ...
    for (size_t imgIndex = 0; imgIndex <= imgEndIndex - imgStartIndex; imgIndex++)
    {
        ...
        if (bFocusOnVehicle)
        {
            /*cv::rectangle(imgGray,vehicleRect,cv::Scalar(255,0,0),1,8,0);
            cv::Mat roi(imgGray, vehicleRect); // will create a reference to the rectangle r of the original image. 
            cv::imshow("ROI", imgGray);*/
            ...
            for (int i = 0; i < keypoints.size(); i++)
            {
                ...
                {
                    // Nothing to do. This keypoint has to be conserved.
                    temp.push_back(keypoints[i]);
                }
                else
                {
                    // Unwanted keypoint, so it is not taken into account
                }
            }
            keypoints.clear();
            keypoints.assign(temp.begin(), temp.end());
        ...
        if (dataBuffer.size() > 1) // wait until at least two images have been processed
        {
            ...
            {
                // SIFT is a HOG detector
                descriptorClass = "DES_HOG";
            }
            else
            {
                // BRIEF, FAST, BRISK, ORB and AKAZE are binary descriptors
